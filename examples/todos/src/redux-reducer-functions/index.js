const reCanBeArray = /^\[[^\]]*]$/g
let cached = {}
const options = {
  defaultReducerMethodName : 'other', //name of the reducer function which used  with standart reduce using  switch/case
  useCache : false,
...
export function combineReducers(reducers, opts) {
  const {defaultReducerMethodName, useCache} = {...options, ...opts}
  return (state = {}, action) => {
    ...
    if (!options.useCache || !cached[type]) {
      if (reducerName && method) {
        if (reCanBeArray.test(reducerName)) {
          targetReducers = JSON.parse(reducerName);
        } else {
    ...
    if (useCache  && cached[type]) {
      ...
      reducerKeys.forEach(key => {
        ...
        const actualReducer = reducers[key][isReducerFunction ? method : defaultReducerMethodName]
